...
\subsection{订单环\label{sec:order_ring}}
在路印协议中,订单由单向数据模型表示(UDOM)。UDOM把每个订单用一个代币交换请求来表示 amountS/amountB(分别为卖和买的数量),而不单纯是买和卖的请求。我们知道每个订单必定是两种货币之间的交换(例如ETH换成USD)，所以这个协议最大的魅力在于,它可以把多个订单混合在一个交易环路中进行撮合。与传统在两个币种间，即一个交易对的买卖两个方向间完成撮合不同，路印在交易环路中最多可以使用16个订单,促使交易整体具有更大的流动性和更小的价差。
